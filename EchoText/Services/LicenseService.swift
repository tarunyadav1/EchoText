...
final class LicenseService: ObservableObject {
    ...
    /// Dev bypass key for testing - REMOVE IN PRODUCTION
    #if DEBUG
    private let devBypassKey = "ECHOTEXT-DEV-2024"
    #endif

    // MARK: - Private Properties
    private let keychainService = "com.echotext.license"
    private let keychainAccount = "license_key"
    private let storedLicenseKey = "com.echotext.stored_license"
    ...
    func activate(licenseKey: String) async throws {
        ...
        // Dev bypass for testing - REMOVE IN PRODUCTION
        #if DEBUG
        if licenseKey == devBypassKey {
            try storeLicenseKey(licenseKey)
            let devLicenseInfo = LicenseInfo(
                email: "dev@echotext.app",
                productName: "EchoText Pro (Dev)",
            ...
            let storedLicense = StoredLicense(
                ...
                activatedAt: Date(),
                lastVerified: Date(),
                licenseInfo: devLicenseInfo
            )
            storeLocalLicense(storedLicense)
            licenseState = .valid(devLicenseInfo)
        ...
        if response.success {
            ...
            licenseState = .invalid(response.message ?? error.localizedDescription ?? "Activation failed")
            throw error
        }
    }
        ...
        do {
            ...
            if response.success {
                ...
                licenseState = .valid(response.licenseInfo ?? storedLicense.licenseInfo ?? LicenseInfo(
                    ...
                    productName: "EchoText Pro",
                    createdAt: nil,
                    activatedAt: response.activatedAt ?? ISO8601DateFormatter().string(from: storedLicense.activatedAt),
                    variants: nil
                ))
            } else {
    ...
    private func callServer(endpoint: String, licenseKey: String, machineId: String) async throws -> LicenseResponse {
...
extension Notification.Name {
    static let licenseActivated = Notification.Name("com.echotext.licenseActivated")
    static let licenseDeactivated = Notification.Name("com.echotext.licenseDeactivated")
}
